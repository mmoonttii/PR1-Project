	...
	Giocatore *initGiocatori(int nGiocatori, CartaCfu **mazzoCfu) {
		...
		for (int i = 0; i < nGiocatori; ++i) {
			...
			if (listaGiocatori == NULL) {
				...
				*listaGiocatori = aux;
				listaGiocatori->nextPlayer = NULL;
				head = listaGiocatori;
				listaGiocatori->listaCarte = distribuisciCarte(mazzoCfu);

			} else {
				head->nextPlayer = allocaGiocatore();
				*head->nextPlayer = aux;
				head->nextPlayer->nextPlayer = NULL;
				head->listaCarte = distribuisciCarte(mazzoCfu);
